<script lang="ts">
	...
	let obj: any;
	if (preset) {
		obj = { value: preset, label: preset };
	}
</script>

<Select.Root selected={obj}>
	<Select.Trigger class="mt-1.5 h-[45px] w-[180px]" aria-label="Select a plan">
		<Select.Input name="plan" />
		<Receipt class="text-white/80" size={14} />
